...
- `src/config/booking.ts` owns reading order, bare `/book/` default (`natal-90`), eligibility labels, and booking actions. Multi-option readings expose their real priced choices.
...
- Repository validation covers build, content, links, interactive flows, accessibility, and performance.
...
- Staging and future production Cloudflare Workers are separate; staging is protected from indexing.
...
- Build booking links through `src/config/booking.ts`; do not hand-write service/event mappings.
